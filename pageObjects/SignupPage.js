export class SignupPage {
  ...
  async webiste_url_Page_going(Website_Url) {
  ...
  async enterOtp(otp) {
    for (let i = 0; i < otp.length; i++) {
      await this.page.locator(this.otpInputs[i]).fill(otp[i]);
    }
  }
  ...
  async enterChildFirstName(children_firstName) {
    await this.childFirstNameInput.fill(children_firstName);
  }

  async enterChildLastName(children_lastName) {
    await this.childLastNameInput.fill(children_lastName);
  }

async selectChildDob(children_dateOfBirth) {
...
// Wait for and click the Grade dropdown, then select 'Pre-Kindergarten'
  ...
  async enterParentFirstName(Parentname) {
    await this.parentFirstNameInput.fill(Parentname);
    
  }

  async enterAddress(doorNo, location) {
